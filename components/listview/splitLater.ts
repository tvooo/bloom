import { endOfToday, format, isBefore, isSameDay, isSameWeek, isTomorrow, nextMonday, nextSaturday, nextSunday, startOfDay } from "date-fns";
import { Task } from "model/task";
import { ensureDate, isScheduled, isScheduledAfter, not } from "utils/filters";
...
const splitLater = (items: Task[]): Array<{ label: string | null; items: Task[] }> => {
  ...
  return [
    { label: null, items: items.filter(not(isScheduledAfter(endOfToday()))) },
    { label: "Later", items: items.filter(isScheduledAfter(endOfToday())) }
  ]
}

export default splitLater;